...
void BpTree::splitDataNode(BpTreeNode* pDataNode) {
	...
	if (pDataNode == root) {
		...
		if (excessIndexNode(newIndexNode)) // indexnode splite
			splitIndexNode(newIndexNode); // splite indexnode
		else // not splited
			return;
	}
...
void BpTree::splitIndexNode(BpTreeNode* pIndexNode) {
	if (pIndexNode == root) {
		...
		iter++; //next
		newIndexNode_right->insertIndexMap(iter->first, iter->second);
		newIndexNode_right->setParent(newIndexNode_left); //set, parent child
...
int BpTree::getFreq(set<string> items) { // get frequency
	int freq = 0;
	BpTreeNode* node = root;
	if (root == NULL) return false;
	while(!dynamic_cast<BpTreeDataNode*>(node)) node = node->getMostLeftChild();
...
bool BpTree::printConfidence(string item, double min_confidence)
{
	...
	for(auto i : freq_list)
	{
		if (threshold < i.second)
		{
			...
			*fout << "} " << i.second << ' ' << (double)i.second / freq << endl;
		}
	}
	return true;
}
bool BpTree::printRange(string item, int min, int max) { // print Range
	bool has_pattern = false;
	BpTreeNode* node = root;
	if (root == NULL) return false;
