...
void FPGrowth::createFPtree(FPNode* root, HeaderTable* table, list<string> item_array, int frequency) {
	...
	list<string>::iterator it = item_array.begin();
	for (it = item_array.begin(); it != item_array.end(); it++) {

		FPNode* newfpnode = new FPNode;
		// string-> char change
		newfpnode->setItem((char*)(*it).c_str());
		//frequency
		newfpnode->updateFrequency(frequency);
		//set
		newfpnode->setParent(parentfpnode);
		
		// search same item which is not exist
